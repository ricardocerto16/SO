...
int main(int argc, char const *argv[]){
	...
	int fdout = open("saida2.txt",O_CREAT | O_WRONLY | O_TRUNC, 0666);
	// adicionamos a flag 0666, para podermos trabalhar com o ficheiro
	int fderr = open("erros2.txt" ,O_CREAT | O_WRONLY | O_TRUNC, 0666);
	...
	// podemos fechar os ficheiros antes do fork pq os 
	// ficheiros passam a ser os 0 e 1.
	close(fdin);
	close(fdout);
	close(fderr);
